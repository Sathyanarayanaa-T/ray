...
# Tests moved from test_operators.py


def _create_map_transformer_from_block_fn(
    block_fn,
...
def test_actor_pool_map_operator_init_from_operators(
    ray_start_regular_shared, data_context_override
):
    ...
    op = MapOperator.create(
        _create_map_transformer_from_block_fn(_sleep, init_fn=_fail),
        input_op=input_op,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            None,
            4,
            4 * 4,  # DEFAULT_ACTOR_MAX_TASKS_IN_FLIGHT_TO_MAX_CONCURRENCY_FACTOR
        ),
    ],
)
def test_actor_pool_map_operator_should_add_input_from_operators(
    ...
    def _failing_transform(block_iter, task_context: TaskContext):
        raise ValueError("expected failure")

    op = MapOperator.create(
        _create_map_transformer_from_block_fn(_failing_transform),
        input_op=input_op,
        data_context=ctx,
        name="TestMapper",
        compute_strategy=compute_strategy,
...
def test_actor_pool_map_operator_num_active_tasks_and_completed_from_operators(
    shutdown_only,
):
    ...
    def _map_transfom_fn(block_iter, _):
    ...
    op = MapOperator.create(
        _create_map_transformer_from_block_fn(_map_transfom_fn),
        input_op=input_op,
        data_context=DataContext.get_current(),
        name="TestMapper",
        compute_strategy=compute_strategy,
